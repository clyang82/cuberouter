...
function saveAndOpenErrorLog() {
  try {
    ...
    // 写入日志
    const logContent = `CubeRouter 崩溃日志
...
function analyzeError(errorLogs) {
  ...
  if (allLogs.includes('database is locked') || 
    ...
    return {
      ...
      solution: '可能的解决方案：\n\n1. 检查是否已经打开了另一个 CubeRouter 窗口\n   - 查看任务栏/Dock 中是否有其他 CubeRouter 图标\n   - 查看系统托盘（Windows）或菜单栏（Mac）中是否有 CubeRouter 图标\n\n2. 如果刚刚关闭过应用，请等待 10 秒后再试\n\n3. 重启电脑以释放被占用的文件\n\n4. 如果问题持续，可以尝试：\n   - 退出所有 CubeRouter 实例\n   - 删除数据目录中的临时文件（.db-shm 和 .db-wal）\n   - 重新启动应用'
    };
  }
