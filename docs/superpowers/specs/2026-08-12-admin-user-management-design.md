...
> Date: 2026-08-12
> Source: `/home/skeeey/workspace/searouter-isuanova` (`docs/admin-user-management.md`)
...
- **User CSV export** — `POST /api/user/export`: streamed CSV of **all** users, either by explicit `ids` or by `keyword` + `group` filter; batch 200; UTF-8 BOM; i18n headers per request locale; 14 columns including role/status labels and `remark`, `aff_code`, `aff_count`, `inviter_id`.
...
- **`ExportUsersByIds(ids []int) ([]*User, error)`** — batches at `exportBatchSize = 200` (`id IN ?` per batch), `Omit("password", "access_token")`; a batch error is logged via `common.SysLog` (`ExportUsers ids batch err: %v`) and **returned** so the caller fails the request (A1 behavior — the ops port's `ExportOpsInviteesByIds` precedent).
- **`ExportUsersByFilter(keyword, group string) ([]*User, error)`** — pages of 200: empty keyword+group → `GetAllUsers`; otherwise `SearchUsers(keyword, group, ...)`. Stops on a short page or `len == 0`; batch error logged (`ExportUsers filter batch err: %v`) and returned. Note: this repo's `GetAllUsers`/`SearchUsers` are `Unscoped()` (include soft-deleted users) — the export matches the users table the export button lives on.
...
Opened from a per-row 邀请记录 action in `data-table-row-actions.tsx`. Paginated dialog table: username / email / phone (raw — no masking) / status / group / role / created_at; loads page 1 on open, resets on close, empty state 暂无邀请记录.
...
6. `ExportUsers` with keyword/group (and empty) — filter results streamed (source doc case 2, log-line dropped).
...
13. `api.ts` additions — `getUserInvitees`/`getUserQuotaDates` query-param building and `exportUsers` payload shape (vitest, matching existing style).
