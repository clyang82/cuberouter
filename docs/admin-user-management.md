...
- `controller/user_quota_dates_test.go` — `TestCanViewUserDashboard` (role matrix), `TestGetUserQuotaDatesByAdminRoleMatrix` (admin sees common/ops, not admin/root; root sees anyone), `TestGetUserQuotaDatesByAdminRangeGuard` (cap and boundary), `TestGetUserQuotaDatesByAdminResponseShape` (`data.user` matches `UserDashboardBrief`, `data.dates` matches `GetQuotaDataByUserId` output), `TestGetUserQuotaDatesByAdminInvalidIdAndMissingUser`.
